...
class SnapAssistContent extends St.BoxLayout {
  ...
  private _applyStyle() {
    ...
    this._bottomPadding =
      ...
      (this.get_theme_node().get_padding(St.Side.BOTTOM) / (alreadyScaled ? finalScalingFactor : 1));
  ...
  public onMovingWindow(window: Meta.Window, ease: boolean = false, currPointerPos: { x: number; y: number }) {
    ...
    if (!this._showing || !this._isEnlarged) {
      ...
      if (wasEnlarged)
        this._container.emit(SNAP_ASSIST_SIGNAL, new Tile({ x: 0, y: 0, width: 0, height: 0, groups: [] }));
      return;
    }
  ...
  private handleOpening(window: Meta.Window, ease: boolean = false, currPointerPos: { x: number; y: number }) {
    ...
    const isNear =
      this.isBetween(
        this._container.x + this.x - this._activationAreaOffset,
        currPointerPos.x,
        this._container.x + this.x + this.width + this._activationAreaOffset,
      ) &&
      this.isBetween(
        this._container.y - this._activationAreaOffset,
        currPointerPos.y,
        this._container.y + this._enlargedVerticalDistance + size + this._activationAreaOffset,
      );
