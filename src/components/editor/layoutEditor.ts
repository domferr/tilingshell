...
export default class LayoutEditor extends St.Widget {
    ...
    private _drawEditor() {
        ...
        groups.forEach((tiles, groupdId) => {
            ...
            let lines = tiles.flatMap(t => [
                {c: Math.round(t.tile.x * 1000) / 1000, end: false, r: t.rect.x}, 
                {c: Math.round((t.tile.x+t.tile.width) * 1000) / 1000, end: true, r: t.rect.x+t.rect.width}
            ...
            if (coord === -1) {
                ...
                lines = tiles.flatMap(t => [
                    {c: Math.round(t.tile.y * 1000) / 1000, end: false, r: t.rect.y}, 
                    {c: Math.round((t.tile.y + t.tile.height) * 1000) / 1000, end: true, r: t.rect.y+t.rect.height}
                ]).sort((a, b) => a.c - b.c !== 0 ? (a.c - b.c):(a.end ? -1:1));
                count = 0;
                for (const line of lines) {
                    count += line.end ? -1:1;
    ...
    private splitTile(editableTile: EditableTilePreview) {
        ...
        if (splitHorizontally) {
            editableTile.getSlider(St.Side.TOP)?.onTileSplit(editableTile, [prevEditableTile, nextEditableTile]);
            editableTile.getSlider(St.Side.BOTTOM)?.onTileSplit(editableTile, [prevEditableTile, nextEditableTile]);
            ...
            editableTile.getSlider(St.Side.LEFT)?.onTileSplit(editableTile, [prevEditableTile, nextEditableTile]);
    ...
    private _buildSlider(isHorizontal: boolean, coord: number, groupId?: number) : Slider {
        if (!groupId) {
