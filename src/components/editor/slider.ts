...
export default class Slider extends St.Button {
  ...
  private _createTileSignals(tile: EditableTilePreview) {
    ...
    this._signals.get(tile)?.push(tile.connect('size-changed', this._onTileSizeChanged.bind(this)));
  ...
  private _move(eventX: number, eventY: number) {
    ...
    if (this._lastEventCoord !== null) {
      ...
      for (const prevTile of this._previousTiles) {
        if (
          prevTile.rect.width + movement.x < EditableTilePreview.MIN_TILE_SIZE ||
          prevTile.rect.height + movement.y < EditableTilePreview.MIN_TILE_SIZE
        )
          return;
      }
      for (const nextTile of this._nextTiles) {
        if (
          nextTile.rect.width - movement.x < EditableTilePreview.MIN_TILE_SIZE ||
          nextTile.rect.height - movement.y < EditableTilePreview.MIN_TILE_SIZE
        )
          return;
      }
